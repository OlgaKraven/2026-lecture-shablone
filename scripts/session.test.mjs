...
import {validMessage,newer} from '../src/engine/session.ts'
import {evaluate} from '../src/engine/scoring.ts'
const scope={session:'one',lecture:'L001',course:'demo',version:'1',base:'/demo/'}
const state={slideId:'s1',black:false,animations:true,replay:0,profile:{fullName:'',position:'',department:''},epoch:10,sequence:1}
const message={protocol:1,session:'one',lecture:'L001',course:'demo',version:'1',type:'STATE',state}
test('public snapshot accepted; private and cross-session data rejected',()=>{assert.equal(validMessage(message,scope),true);assert.equal(validMessage({...message,session:'other'},scope),false);assert.equal(validMessage({...message,state:{...state,notes:'private'}},scope),false);assert.equal(validMessage({...message,state:{...state,profile:{...state.profile,answer:'private'}}},scope),false)})
test('duplicate and old snapshots ignored; new epoch accepted',()=>{assert.equal(newer(state,state),false);assert.equal(newer({...state,sequence:0},state),false);assert.equal(newer({...state,epoch:11,sequence:0},state),true)})
