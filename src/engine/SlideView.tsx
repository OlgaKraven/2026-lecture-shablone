...
export function SlideView({course,lecture,slide,profile,base,number,interactive=false}:{course:Course;lecture:Lecture;slide:Slide;profile:Profile;base:string;number:number;interactive?:boolean}){
 ...
 </div>{slide.kind==='section'&&<img className="topic-arrow" src={assetUrl(course.topicArrow||'brand/topic-arrow.webp',base)} alt=""/>}{slide.kind==='title'&&<img className="title-mascot" src={assetUrl(course.mascot,base)} alt={course.mascotAlt}/>}</div>
