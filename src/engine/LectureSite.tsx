...
import {ArrowLeft,ArrowRight,BarChart3,BookOpen,Download,ExternalLink,List,Maximize,Minimize,MonitorPlay,MousePointer2,Moon,Pause,Play,RotateCcw,Search,Sun,UserRound} from 'lucide-react'
...
export function LectureSite({course,base='/'}:{course:Course;base?:string}){
 ...
 <footer className="catalog-footer"><span>{course.demo?'Демонстрационный курс · образцы макетов, не полный комплект по РПД':course.discipline}</span><span>Учебный год {course.year}</span></footer></main>:<main className={`deck-shell ${animations?'motion-on':''}`}>
